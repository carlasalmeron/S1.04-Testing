...
2. **Environment Variables:**
   Create a `.env` file with any specific local configurations (e.g., database connection details if added in future exercises).
3. **Execution:**
   Navigate to the exercise level (e.g., `Level1`) and compile the project:
   ```bash
   mvn compile
   ```
4. **Tests:**
   To run the tests and verify implementation, execute:
   ```bash
   mvn test
   ```

## 🧩 Diagrams and justification of technical decisions
- **Maven Multi-module**: The project is structured into levels to isolate basic JUnit tests (Level 1) from advanced AssertJ assertions (Level 2).
- **JUnit 5 / AssertJ**: Combined to provide a robust testing suite with both standard and fluent assertion styles.